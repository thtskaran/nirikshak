...
const DeploymentModal = ({ isOpen, step, isSuccess, isError, onClose }: DeploymentModalProps) => {
  const getProgressValue = () => {
    ...
    const stepKeywords = [
      'Initializing', 'Connecting', 'Pulling', 'Creating', 'Downloading',
      'Configuring', 'Starting', 'Provisioning', 'Verifying', 'Finalizing'
    ];
          ...
          <div className="text-center">
            ...
            {!isSuccess && !isError && (
              <div className="text-gray-400 text-sm">
                {step.includes('Docker') && 'Setting up containerization environment...'}
                {step.includes('Ollama') && 'Preparing AI model runtime...'}
                {step.includes('Downloading') && 'This may take a few minutes...'}
                {step.includes('Provisioning') && 'Allocating system resources...'}
                {!step.includes('Docker') && !step.includes('Ollama') && !step.includes('Downloading') && !step.includes('Provisioning') && 
